...
GamePacket PacketFactory::createEnterStartingPacket(
	int map, 
	const std::vector<std::tuple<int, std::string, int>>& users)
{
	flatbuffers::FlatBufferBuilder builder;
	
	std::vector<flatbuffers::Offset<Data::InGameUserData>> fvec;
	for (auto e : users)
	{
		int id = std::get<0>(e);
		string name = std::get<1>(e);
		int character = std::get<2>(e);

		auto fname = builder.CreateString(name);
		auto fuser = Data::CreateUserData(builder, id, fname);
		auto fingame = Data::CreateInGameUserData(builder, fuser, character);
		fvec.push_back(fingame);
	}
	auto ingame_users_data = builder.CreateVector(fvec);
	auto data = Data::CreateWorldData(builder, ingame_users_data, map);
	builder.Finish(data);

	return GamePacket(builder, kEnterStarting);
}