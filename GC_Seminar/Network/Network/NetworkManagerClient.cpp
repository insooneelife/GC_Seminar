...
void NetworkManagerClient::processInput(char ch)
{
	...
	if (ch == 13)
	{
		...
		else if (_input_message == "--create room")
		{
			GamePacket& packet =
				PacketFactory::createCreateRoomPacket(_id, _client_name);
			send(packet, *_server_address);
		}
		// #We need some input system for this.
		else if (_input_message == "--join room 1")
		{
			GamePacket& packet = PacketFactory::createJoinRoomPacket(1);
			send(packet, *_server_address);
		}
		else if (_input_message == "--join room 2")
		{
			GamePacket& packet = PacketFactory::createJoinRoomPacket(2);
			send(packet, *_server_address);
		}
		else if (_input_message == "--show room")
		{
